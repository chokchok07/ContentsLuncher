...
1. **기기 생존 및 동작 상태 확인 (10초 간격)**: 
   * 시연 컴퓨터는 10초에 한 번씩 "나 잘 살아있어"라는 신호와 함께 현재 "시연 콘텐츠가 실행 중인지", 아니면 "대기 중인지"의 상태를 중앙 컨트롤러로 보냅니다. 이를 통해 관리자는 단순히 컴퓨터 전원만 켜진 것인지, 실제 전시 프로그램까지 잘 돌아가고 있는지 한눈에 알 수 있습니다.
2. **에이전트 검사**:
